...
Version: v5 (RF-DETR single-model worker)
...
2.  Detection pipeline:
      • Person detection  — RF-DETR full-frame Person-only pass
      • PPE violation     — RF-DETR full-frame pass, plus per-person crop recovery
      • Ownership scoring — assigns crop detections back to the best person
...
Environment variables (all optional — defaults shown):
  ...
  PERSON_MODEL_PATH           ./checkpoint_best_ema.pth
  PPE_MODEL_PATH              ./checkpoint_best_ema.pth
  ...
  PERSON_MIN_BOX_WIDTH        8.0
  PERSON_MIN_BOX_HEIGHT       20.0
...
DETECTOR_BACKEND  = _env("DETECTOR_BACKEND", "rfdetr").strip().lower()
RFDETR_MODEL_SIZE = _env("RFDETR_MODEL_SIZE", "medium").strip().lower()
RFDETR_OPTIMIZE_INFERENCE = _env_bool("RFDETR_OPTIMIZE_INFERENCE", True)
PPE_MODEL_PATH    = _env("PPE_MODEL_PATH",    "./checkpoint_best_ema.pth")
...
DETECTION_CONFIDENCE = _env_float("DETECTION_CONFIDENCE", 0.05)
PERSON_CONFIDENCE    = _env_float("PERSON_CONFIDENCE",    0.25)
PERSON_IOU           = _env_float("PERSON_IOU",           0.70)
PERSON_IMAGE_SIZE_MAX        = _env_int("PERSON_IMAGE_SIZE",        1024)
PERSON_AUGMENT_MAX_SCALE     = _env_int("PERSON_AUGMENT_MAX_SCALE", 1920)
PERSON_FALLBACK_ENABLED      = _env_bool("PERSON_FALLBACK_ENABLED",      False)
...
ENABLE_CROWD_RECOVERY        = _env_bool("ENABLE_CROWD_RECOVERY",        False)
...
ENABLE_TILED_PERSON_DETECTION = _env_bool("ENABLE_TILED_PERSON_DETECTION", False)
...
IMAGE_SIZE_MAX = _env_int("IMAGE_SIZE", 1024)
CLASS_CONFIDENCE_THRESHOLDS = parse_class_confidences(
    _env("CLASS_CONFIDENCES",
         "NO-Hardhat=0.10,NO-Safety Vest=0.20,NO-Safety Boots=0.10")
)
MODEL_INFERENCE_CONFIDENCE = min(
    [DETECTION_CONFIDENCE, *CLASS_CONFIDENCE_THRESHOLDS.values()]
    or [DETECTION_CONFIDENCE]
)
CLOSE_PERSON_CONF_ENABLED = _env_bool("CLOSE_PERSON_CONF_ENABLED", True)
CLOSE_PERSON_HEIGHT_RATIO = _env_float("CLOSE_PERSON_HEIGHT_RATIO", 0.60)
CLOSE_PERSON_AREA_RATIO   = _env_float("CLOSE_PERSON_AREA_RATIO",   0.22)
CLOSE_PERSON_CONF_BOOST   = _env_float("CLOSE_PERSON_CONF_BOOST",   0.05)
CLOSE_PERSON_CONF_MAX     = _env_float("CLOSE_PERSON_CONF_MAX",     0.35)

NO_HARDHAT_GEOMETRY_ENABLED = _env_bool("NO_HARDHAT_GEOMETRY_ENABLED", True)
NO_HARDHAT_PRIMARY_ZONE     = _env_float("NO_HARDHAT_PRIMARY_ZONE",     0.45)
NO_HARDHAT_FALLBACK_ZONE    = _env_float("NO_HARDHAT_FALLBACK_ZONE",    0.70)
NO_HARDHAT_PRIMARY_CONF     = _env_float("NO_HARDHAT_PRIMARY_CONF",     0.20)
NO_HARDHAT_FALLBACK_CONF    = _env_float("NO_HARDHAT_FALLBACK_CONF",    0.30)
NO_HARDHAT_BYPASS_CONF      = _env_float("NO_HARDHAT_BYPASS_CONF",      0.40)
NO_HARDHAT_MIN_WIDTH_RATIO  = _env_float("NO_HARDHAT_MIN_WIDTH_RATIO",  0.06)
NO_HARDHAT_MIN_HEIGHT_RATIO = _env_float("NO_HARDHAT_MIN_HEIGHT_RATIO", 0.06)
NO_HARDHAT_PARTIAL_EDGE_MARGIN = _env_float("NO_HARDHAT_PARTIAL_EDGE_MARGIN", 0.02)
NO_HARDHAT_PARTIAL_ZONE        = _env_float("NO_HARDHAT_PARTIAL_ZONE",        0.90)
NO_HARDHAT_PARTIAL_CONF        = _env_float("NO_HARDHAT_PARTIAL_CONF",        0.20)
NO_HARDHAT_PARTIAL_MIN_WIDTH_RATIO  = _env_float("NO_HARDHAT_PARTIAL_MIN_WIDTH_RATIO",  0.04)
NO_HARDHAT_PARTIAL_MIN_HEIGHT_RATIO = _env_float("NO_HARDHAT_PARTIAL_MIN_HEIGHT_RATIO", 0.04)
...
PERSON_BOX_DEDUPE_IOU    = _env_float("PERSON_BOX_DEDUPE_IOU",    0.50)
PERSON_BOX_DEDUPE_COVERAGE = _env_float("PERSON_BOX_DEDUPE_COVERAGE", 0.95)
PERSON_MIN_BOX_WIDTH     = _env_float("PERSON_MIN_BOX_WIDTH",     8.0)
PERSON_MIN_BOX_HEIGHT    = _env_float("PERSON_MIN_BOX_HEIGHT",    15.0)
PERSON_MIN_ASPECT_RATIO  = _env_float("PERSON_MIN_ASPECT_RATIO",  0.15)
PERSON_MAX_ASPECT_RATIO  = _env_float("PERSON_MAX_ASPECT_RATIO",  15.00)
NO_VIOLATION_MIN_PERSON_HEIGHT = _env_int("NO_VIOLATION_MIN_PERSON_HEIGHT", 45)
NO_VIOLATION_RECHECK_ENABLED = _env_bool("NO_VIOLATION_RECHECK_ENABLED", False)
...
NO_VIOLATION_RECHECK_MIN_HEIGHT_RATIO = _env_float("NO_VIOLATION_RECHECK_MIN_HEIGHT_RATIO", 0.20)

# F3 — per-person crop PPE inference
ENABLE_CROP_PPE         = _env_bool("ENABLE_CROP_PPE",         True)
PPE_CROP_INFERENCE_SIZE = _env_int("PPE_CROP_INFERENCE_SIZE",   1024)
PPE_CROP_MARGIN         = _env_float("PPE_CROP_MARGIN",         0.15)
PPE_OWNER_MIN_OVERLAP   = _env_float("PPE_OWNER_MIN_OVERLAP",   0.20)
PPE_CROP_DUPLICATE_IOU  = _env_float("PPE_CROP_DUPLICATE_IOU",  0.35)
...
def is_close_person_box(person_box: Tuple, fw: int, fh: int) -> bool:
    if fw <= 0 or fh <= 0:
        return False
    x1, y1, x2, y2 = person_box
    w = max(0.0, x2 - x1)
    h = max(0.0, y2 - y1)
    height_ratio = h / float(fh)
    area_ratio = (w * h) / float(fw * fh)
    return height_ratio >= CLOSE_PERSON_HEIGHT_RATIO or area_ratio >= CLOSE_PERSON_AREA_RATIO


def get_adaptive_class_confidence_threshold(
    class_name: str,
    det: Dict,
    ...
    base = get_class_confidence_threshold(class_name)
    if not CLOSE_PERSON_CONF_ENABLED:
        return base
    pidx = det.get("person_index")
    if not isinstance(pidx, int) or pidx < 0 or pidx >= len(person_boxes):
        return base
    if not is_close_person_box(person_boxes[pidx], fw, fh):
        return base
    return min(max(base + CLOSE_PERSON_CONF_BOOST, base), CLOSE_PERSON_CONF_MAX)


def filter_detections_by_adaptive_confidence(
    ...
    for det in detections:
        threshold = get_adaptive_class_confidence_threshold(
            det["class_name"],
            det,
            person_boxes,
            fw,
    ...
    if suppressed:
        print(
            f"[CONF] Close-person adaptive threshold suppressed {suppressed} detection(s)",
            flush=True,
        )
    return kept


def is_truncated_person_box(person_box: Tuple, fw: int, fh: int) -> bool:
    if fw <= 0 or fh <= 0:
        return False
    x1, y1, x2, y2 = person_box
    x_margin = fw * NO_HARDHAT_PARTIAL_EDGE_MARGIN
    y_margin = fh * NO_HARDHAT_PARTIAL_EDGE_MARGIN
    return (
        x1 <= x_margin
        or y1 <= y_margin
        or x2 >= fw - x_margin
...
def is_valid_no_hardhat_geometry(
    det: Dict,
    person_boxes: List[Tuple],
    fw: int = 0,
    fh: int = 0,
) -> bool:
    if det.get("class_name") != "NO-Hardhat" or not NO_HARDHAT_GEOMETRY_ENABLED:
        return True

    conf = float(det.get("confidence", 0.0))
    if conf >= NO_HARDHAT_BYPASS_CONF:
        return True
    ...
    dw = max(0.0, dx2 - dx1)
    dh = max(0.0, dy2 - dy1)
    y_frac = _vertical_center_fraction(det["box"], person_box)
    is_partial_person = is_truncated_person_box(person_box, fw, fh)
    if is_partial_person:
        min_w = NO_HARDHAT_PARTIAL_MIN_WIDTH_RATIO
        min_h = NO_HARDHAT_PARTIAL_MIN_HEIGHT_RATIO
        if dw / pw < min_w or dh / ph < min_h:
            return False
        if (
            not box_center_inside(det["box"], person_box)
            and box_overlap_fraction(det["box"], person_box) < PPE_OWNER_MIN_OVERLAP
        ):
            return False
        return y_frac <= NO_HARDHAT_PARTIAL_ZONE and conf >= NO_HARDHAT_PARTIAL_CONF

    if dw / pw < NO_HARDHAT_MIN_WIDTH_RATIO or dh / ph < NO_HARDHAT_MIN_HEIGHT_RATIO:
        return False

    if y_frac <= NO_HARDHAT_PRIMARY_ZONE:
        return conf >= NO_HARDHAT_PRIMARY_CONF
    if y_frac <= NO_HARDHAT_FALLBACK_ZONE:
        return conf >= NO_HARDHAT_FALLBACK_CONF
    return False


def filter_no_hardhat_by_geometry(
    detections: List[Dict],
    person_boxes: List[Tuple],
    fw: int = 0,
    fh: int = 0,
    ...
    suppressed = 0
    for det in detections:
        if is_valid_no_hardhat_geometry(det, person_boxes, fw, fh):
            kept.append(det)
        else:
            suppressed += 1
            debug(
                "[HARDHAT] Geometry suppressed "
                f"conf={det.get('confidence', 0.0):.3f} "
                f"pidx={det.get('person_index')} box={det.get('box')}"
            )
    if suppressed:
        print(
            f"[HARDHAT] Geometry suppressed {suppressed} NO-Hardhat detection(s)",
            flush=True,
        )
    return kept
...
_conf_histogram: Dict[str, List[float]] = {}


def log_raw_confidence(class_name: str, conf: float) -> None:
    if DEBUG:
        _conf_histogram.setdefault(class_name, []).append(float(conf))


def print_confidence_histogram() -> None:
    if not DEBUG or not _conf_histogram:
        return
    for cname, confs in sorted(_conf_histogram.items()):
        if not confs:
            continue
        arr = np.asarray(confs, dtype=np.float32)
        threshold = get_class_confidence_threshold(cname)
        below = int((arr < threshold).sum())
        print(
            f"[CONF HIST] {cname}: n={len(arr)} "
            f"mean={arr.mean():.3f} p50={np.median(arr):.3f} "
            f"p95={np.percentile(arr, 95):.3f} "
            f"below_threshold={below} ({100 * below / len(arr):.0f}%)",
            flush=True,
        )
...
class _NormalizedResult:
    def __init__(
        ...
        self.boxes = _NormalizedBoxes(xyxy, cls, conf)


class RFDetrDetectorAdapter:
    """Expose RF-DETR predictions through the small YOLO-like surface this worker uses."""

    def __init__(self, model_path: str, size: str, device: Any = "cpu") -> None:
        try:
            import rfdetr
        except ImportError as exc:
            raise ImportError(
                "RF-DETR is not installed. Install the RF-DETR Docker/runtime dependencies."
            ) from exc
        ...
        class_name = {
            ...
            "medium": "RFDETRMedium",
            "large": "RFDETRLarge",
        }.get(size, "RFDETRMedium")
        model_class = getattr(rfdetr, class_name, None) or getattr(rfdetr, "RFDETRMedium")
        if not hasattr(model_class, "from_checkpoint"):
            raise RuntimeError("Installed RF-DETR package does not expose from_checkpoint().")

        self.device = self._normalize_device(device)
        self.model = model_class.from_checkpoint(str(model_path), device=self.device)
        model_ctx = getattr(self.model, "model", None)
        if model_ctx is not None and hasattr(model_ctx, "device"):
            model_ctx.device = torch.device(self.device)
        raw_names = getattr(self.model, "class_names", None)
        if isinstance(raw_names, dict):
            self.names = {int(k): str(v) for k, v in raw_names.items()}
        elif isinstance(raw_names, (list, tuple)):
            self.names = {int(i): str(v) for i, v in enumerate(raw_names)}
        else:
            self.names = dict(DEFAULT_RFDETR_CLASS_NAMES)

        if RFDETR_OPTIMIZE_INFERENCE and hasattr(self.model, "optimize_for_inference"):
            try:
                self.model.optimize_for_inference()
            except Exception as exc:
                print(f"[MODEL WARN] RF-DETR optimize_for_inference failed: {exc}", flush=True)
    ...
    def _normalize_class_id(self, raw_id: int) -> int:
        if raw_id in self.names:
            return raw_id
        if raw_id - 1 in self.names:
            return raw_id - 1
        return raw_id
    ...
    def predict(
        ...
        **kwargs,
        ...
        agnostic_nms = bool(kwargs.pop("agnostic_nms", False))
        ...
        allowed = set(int(c) for c in classes) if classes is not None else None
        xyxy_out: List[Tuple[float, float, float, float]] = []
        cls_out: List[int] = []
        conf_out: List[float] = []
        ...
        for box, raw_class_id, score in zip(xyxy, class_ids, scores):
            class_id = self._normalize_class_id(int(raw_class_id))
            cname = self.names.get(class_id, str(class_id))
            log_raw_confidence(cname, float(score))
            if allowed is not None and class_id not in allowed:
                continue
            cval = float(score)
            if cval < conf:
                continue
            xyxy_out.append(tuple(float(v) for v in box[:4]))
            cls_out.append(class_id)
            conf_out.append(cval)
        xyxy_out, cls_out, conf_out = self._nms(
            xyxy_out,
            cls_out,
            conf_out,
            iou_threshold=iou,
        ...
        return [_NormalizedResult(xyxy_out, cls_out, conf_out)]
    ...
    def _nms(
        ...
        try:
            ...
            boxes_t = torch.tensor(xyxy, dtype=torch.float32)
            scores_t = torch.tensor(conf, dtype=torch.float32)
            if not agnostic:
                class_t = torch.tensor(cls, dtype=torch.float32)
                max_coord = boxes_t.max()
                boxes_t = boxes_t + (class_t * (max_coord + 1.0))[:, None]
            keep = tv_ops.nms(boxes_t, scores_t, iou_threshold)
            indices = keep.tolist()
        except Exception:
            indices = []
            for idx in sorted(range(len(xyxy)), key=lambda i: conf[i], reverse=True):
                if any(
                    (agnostic or cls[idx] == cls[kept_idx])
                    and box_iou(xyxy[idx], xyxy[kept_idx]) >= iou_threshold
                    for kept_idx in indices
                ):
                    continue
                indices.append(idx)
        return (
            [xyxy[i] for i in indices],
            [cls[i] for i in indices],
            [conf[i] for i in indices],
        )
...
def _vertical_center_fraction(box: Tuple, person_box: Tuple) -> float:
...
def _class_region_bonus(class_name: str, y_frac: float) -> float:
    if class_name == "NO-Hardhat":
        return 0.25 if y_frac <= 0.45 else -0.15
    if class_name == "NO-Safety Boots":
        return 0.25 if y_frac >= 0.55 else -0.15
    if class_name == "NO-Safety Vest":
        return 0.20 if 0.15 <= y_frac <= 0.85 else -0.10
    return 0.0


def compute_detection_person_ownership(
    det_box: Tuple,
    person_box: Tuple,
    class_name: str,
) -> Tuple[float, float, bool]:
    overlap = box_overlap_fraction(det_box, person_box)
    center_inside = box_center_inside(det_box, person_box)
    y_frac = _vertical_center_fraction(det_box, person_box)
    score = overlap
    if center_inside:
        score += 0.35
    score += _class_region_bonus(class_name, y_frac)
    return score, overlap, center_inside


def assign_crop_detection_owner(
    det: Dict,
    person_boxes: List[Tuple],
    crop_owner_idx: int,
    reference_box: Optional[Tuple] = None,
) -> Optional[Dict]:
    if not person_boxes:
        return None
    ...
    for pidx, person_box in enumerate(person_boxes):
        ownership_box = reference_box if pidx == crop_owner_idx and reference_box is not None else person_box
        score, overlap, center_inside = compute_detection_person_ownership(
            det["box"],
            ownership_box,
            det["class_name"],
        )
        if pidx == crop_owner_idx:
            score += 0.05
        if score > best_score:
            best_idx = pidx
            best_score = score
    ...
    if best_idx is None:
        return None
    if best_overlap < PPE_OWNER_MIN_OVERLAP and not best_center_inside:
        return None

    owned = dict(det)
    owned["person_index"] = best_idx
    owned["crop_owner_index"] = crop_owner_idx
    owned["ownership_score"] = float(best_score)
    return owned


def resolve_cross_crop_duplicates(detections: List[Dict]) -> List[Dict]:
    kept: List[Dict] = []
    for det in sorted(
        detections,
        key=lambda d: (d.get("ownership_score", 0.0), d["confidence"]),
        reverse=True,
    ):
        if not any(
            det["class_name"] == existing["class_name"]
            and det.get("person_index") == existing.get("person_index")
            and box_iou(det["box"], existing["box"]) >= PPE_CROP_DUPLICATE_IOU
...
def dedupe_detections(detections: List[Dict]) -> List[Dict]:
    ...
    for det in sorted(detections, key=lambda d: d["confidence"], reverse=True):
        if not any(
            ...
            and det.get("person_index") == k.get("person_index")
...
def detect_ppe_on_crops(
    ...
    for pidx, pb in enumerate(person_boxes):
        crop, (cx1, cy1, cx2, cy2) = expanded_person_crop(frame, pb, PPE_CROP_MARGIN)
        if crop is None:
            continue
        expanded_reference_box = (float(cx1), float(cy1), float(cx2), float(cy2))
        ...
        for box, cls_id, conf in zip(
            ...
            owned = assign_crop_detection_owner(
                candidate,
                person_boxes,
                pidx,
                reference_box=expanded_reference_box,
            )
            if owned is not None:
                crop_dets.append(owned)

    return resolve_cross_crop_duplicates(crop_dets)
...
def recheck_no_violation_candidates(
    ...
    added_violations = 0
    suppressed = 0
    fh = frame.shape[0] if getattr(frame, "shape", None) is not None else 0
    for pidx, p_box in enumerate(person_boxes):
        ...
        if not NO_VIOLATION_RECHECK_ENABLED:
            ...
            continue
        person_height_ratio = (p_box[3] - p_box[1]) / max(1.0, float(fh))
        if person_height_ratio < NO_VIOLATION_RECHECK_MIN_HEIGHT_RATIO:
            confirmed_no_violation[pidx] = tuple(float(v) for v in p_box)
            continue
...
class IdentityStore:
    ...
    def match(self, embedding: np.ndarray) -> Tuple[Optional[int], float]:
        ...
        top_k       = min(20, len(self._mean_ids))
...
def init_models() -> None:
    ...
    if DETECTOR_BACKEND not in {"yolo", "rfdetr"}:
        raise ValueError(f"Unsupported DETECTOR_BACKEND={DETECTOR_BACKEND!r}")

    same_person_ppe_model = DETECTOR_BACKEND == "rfdetr" or SINGLE_MODEL_MODE or (
    ...
    if DETECTOR_BACKEND == "rfdetr":
        print(
            f"[MODEL] Loading RF-DETR {RFDETR_MODEL_SIZE} Person+PPE model: {PPE_MODEL_PATH}",
            flush=True,
        )
        ppe_model = RFDetrDetectorAdapter(PPE_MODEL_PATH, RFDETR_MODEL_SIZE, device=device)
        ppe_class_names = {int(k): str(v) for k, v in ppe_model.names.items()}
        person_model = ppe_model
        person_class_names = ppe_class_names
    elif same_person_ppe_model:
    ...
    if DETECTOR_BACKEND == "yolo" and not same_person_ppe_model:
...
def detect_persons(frame: np.ndarray, device) -> List[Tuple]:
    ...
    def _run(imgsz: int, conf: float, augment: bool) -> List[Tuple]:
        use_aug = (
            augment
            and imgsz <= PERSON_AUGMENT_MAX_SCALE
            and model_config.get("detector_backend") == "yolo"
        )
        res = model_config["person_model"].predict(
...
def detect_and_reid(
    ...
    for meta in frame_metadata_list:
        ...
        # ── Step 2: Full-frame PPE detection, same as ppe_worker_4 ────────
        ...
        # ── Step 6: Close-person adaptive confidence ─────────────────────
        raw_dets = filter_detections_by_adaptive_confidence(raw_dets, person_boxes, fw, fh)

        # ── Step 7: NO-Hardhat head-zone validation ──────────────────────
        raw_dets = filter_no_hardhat_by_geometry(raw_dets, person_boxes, fw, fh)

        # ── Step 8: Boot color suppression ───────────────────────────────
        raw_dets = suppress_boot_violations_by_color(frame, raw_dets, person_boxes)

        # ── Step 9: Deduplication (F5 — DEDUPE_IOU=0.35) ─────────────────
        raw_dets = dedupe_detections(raw_dets)

        # ── Step 10: Recheck would-be NO-Violation crops before persistence ─
        ...
        # ── Step 10: Full-frame face detection ───────────────────────────
        ...
        # ── Step 11: Group violations by person index ─────────────────────
        person_to_violations: Dict[int, List[Dict]] = {}
        for det in raw_dets:
            pidx = det.get("person_index", -1)
            person_to_violations.setdefault(pidx, []).append(det)

        # ── Step 12: Re-ID every detected person, plus unmatched violation groups.
        ...
        for pidx in sorted(reid_targets):
            ...
            if violations and reid["person_id"] is not None:
                id_store.increment_violations(reid["person_id"], 1)
...
def initialize() -> bool:
    ...
    print("=" * 60, flush=True)
    print("OfficeLens PPE + FaceNet Re-ID Worker  [v5 RF-DETR single-model]", flush=True)
    print("=" * 60, flush=True)
    ...
    p("RF-DETR model size",             RFDETR_MODEL_SIZE if DETECTOR_BACKEND == "rfdetr" else "n/a")
    p("RF-DETR optimized inference",    RFDETR_OPTIMIZE_INFERENCE if DETECTOR_BACKEND == "rfdetr" else "n/a")
    p("--- Detection fixes ---",        "")
    ...
    p("  Crop owner min overlap",       f"{PPE_OWNER_MIN_OVERLAP:.0%}")
    p("  Crop duplicate IoU",           PPE_CROP_DUPLICATE_IOU)
    ...
    p("  Recheck min height ratio",     f"{NO_VIOLATION_RECHECK_MIN_HEIGHT_RATIO:.0%}")
    p("NO-Hardhat geometry gate",       NO_HARDHAT_GEOMETRY_ENABLED)
    p("  Head zones primary/fallback",  f"{NO_HARDHAT_PRIMARY_ZONE:.0%}/{NO_HARDHAT_FALLBACK_ZONE:.0%}")
    p("  Conf primary/fallback/bypass", f"{NO_HARDHAT_PRIMARY_CONF}/{NO_HARDHAT_FALLBACK_CONF}/{NO_HARDHAT_BYPASS_CONF}")
    p("  Partial body zone/conf",       f"{NO_HARDHAT_PARTIAL_ZONE:.0%}/{NO_HARDHAT_PARTIAL_CONF}")
